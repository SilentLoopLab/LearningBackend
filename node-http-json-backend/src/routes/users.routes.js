...
const usersRoutes = async (req, res, endpoint, id) => {
  ...
  if (method === 'GET') {
    ...
    if (id !== undefined) {
      ...
      user ? sendResponse(res, statusCode.Ok, user) : sendResponse(res, statusCode.Not_Found, {
        status: 404,
        error: "Not Found",
    ...
    try {
      ...
      if (!name || !email) {
        sendResponse(res, statusCode.Bad_Request, { error: "Invalid JSON body" });
        return;
      } else {
        role = (role !== 'user' && role !== 'admin') ? 'user' : role;
        const allUsers = readJSON(endpoint);
        if (allUsers.find(u => u.email === email)) {
          sendResponse(res, statusCode.Bad_Request, { message: "Email must be unique" })
          return;
        } else {
          const id = uniqID();
          const createdAt = updatedAt = dateNow();
          const newUser = {
            id,
            name,
            email,
    ...
    try {
      let { name, email, role } = await bodyParser(req);
      if (!name || !email || !role) {
        sendResponse(res, statusCode.Bad_Request, {message: "Invalid JSON body" });
        return;
      } else {
        role = (role !== 'user' && role !== 'admin') ? 'user' : role;
        const allUsers = readJSON(endpoint);
        ...
        if (!putUser) {
          sendResponse(res, statusCode.Not_Found, { message: `User with id ${id} not found` });
          return;
        }
        const emailTaken = allUsers.some(u => u.email === email && u.id !== id);
        if (emailTaken) {
          sendResponse(res, statusCode.Bad_Request, { message: "Email must be unique" });
          return;
        }
        const newUsers = allUsers.map(u => u.id === id ? {
          ...u,
          name,
          email,
          role,
        ...
        sendResponse(res, statusCode.Ok, {...putUser, name, email, role, updatedAt: dateNow()});
        return;
      }
    } catch {
      sendResponse(res, statusCode.Bad_Request, { message: "Invalid JSON body" });
    ...
    try {
        ...
        sendResponse(res, statusCode.Bad_Request,
          {
            status: 400,
            error: "Bad Request",
            message: "Request body is empty. At least one field is required for update."
          }
        );
        return;
      }
      ...
      const newUser = {
        ...
        role: role === undefined
        ? patchUser.role
        : (role === 'user' || role === 'admin' ? role : patchUser.role),
        updatedAt: dateNow(),
      }
      const newUsers = allUsers.map(u => u.id === id ? { ...newUser} : u)
      writeJSON(endpoint, newUsers);
    ...
    if (id === undefined) {
      sendResponse(res, statusCode.Bad_Request, { message: "User id is required" });
      return;
    }
    const allUsers = readJSON(endpoint);
    ...
    sendResponse(res, statusCode.Method_Not_Allowed, { message: 'Merhod Not Allowed' });
  }
}
